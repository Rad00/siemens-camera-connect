...
def writeInt(db_number, start_offset, bit_offset, value):
    reading = plc.db_read(db_number, start_offset, 1)
    snap7.util.set_int(reading, 0, bit_offset, int(value))
    plc.db_write(db_number, start_offset, reading)

# Camera color detection
cap = cv2.VideoCapture(0)  # Use external camera
...
def update_frame():
    ret, frame = cap.read()
    if not ret:
        status_label.config(text="Status: Camera error", fg="red")
        append_status("Camera error")
        return
    ...
    # Detect colors
    blue_detected, _, _ = find_color(frame, blue_lower, blue_upper)
    red_detected, _, _ = find_color(frame, red_lower, red_upper)

    # Determine orientation and send data to PLC
    if blue_detected:
        orientation = "Correct orientation (blue label)"
        orientation_color = "green"
        writeInt(db_number, start_offset, bit_offset, 1)  # Send 1 to PLC
    elif red_detected:
        orientation = "Incorrect orientation (red label)"
        orientation_color = "red"
        writeInt(db_number, start_offset, bit_offset, 2)  # Send 2 to PLC
    else:
        orientation = "Object not detected"
        orientation_color = "orange"
        writeInt(db_number, start_offset, bit_offset, 0)  # Send 0 to PLC
